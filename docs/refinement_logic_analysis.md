...
- **전파 방향**:
  ...
  - **상향(Upward - 동적 역추적)**: 타겟 노드가 참조하는 `mapped_` 식별자의 정의처(부모 블록)를 추적하여 정밀 전파.
  - **계층 Fallback**: 구조적 변경(`Add`/`Delete`) 시에는 계층 구조를 따라 상위 모듈로 전파.
- **시맨틱 필터링**: 무분별한 전파를 방지하기 위해 인텐트와 대상 노드 간의 유사도를 측정 (임계값: **0.1**).
- **기술 기반**: `artifact_mapping` 테이블을 통한 `O(1)` 역참조 및 정방향 정의처 검색.
...
- **영향 기록**: `STALE`로 전환되지 않더라도 영향권에 있는 노드는 `last_action`에 `[Impacted]` 상태와 전파 경로가 기록됩니다.

### Phase 5: 패치 생성 및 검증 (Patch Generation & Validation)
- **목적**: `STALE` 노드에 대해 구체적인 수정안(JSON Patch) 생성 및 정합성 검증.
- **패치 생성 컨텍스트**:
  - **코멘트 통합**: 해당 노드에 남겨진 미해결 사용자 코멘트를 JSON Path와 연동하여 반영.
  - **경로 핀포인팅(Path Pinpointing)**: 수정이 필요한 정확한 JSON 경로 힌트를 AI에게 제공하여 정확도 극대화.
- **자동 검증(AI Evaluation)**:
  - 생성된 패치를 대상으로 AI 평가 수행 (점수 임계값: **80점**, 치명적 오류 0건).
  - 검증 결과와 상관없이 사용자가 최종 판단할 수 있도록 `REVIEW_PENDING` 상태로 전환.

### Phase 6: 최종 승인 및 동기화 (Finalization & Sync)
- **목적**: 사용자가 검토한 변경 사항을 시스템 전체에 반영하고 다음 단계 활성화.
- **동기화 로직**:
  - **아티팩트 매핑 갱신**: 수정된 내용에서 추출된 새로운 `Canonical ID`들을 `artifact_mapping` 테이블에 동기화.
  - **DAG 후행 노드 트리거**: 노드가 `REVIEWED` 상태가 되면 종속된 하위 노드들을 `READY` 상태로 자동 전환.
  - **인텐트 종료**: 모든 작업 완료 시 프로젝트의 `increment_intent`를 클리어하여 세션 종료.
...
| 항목 | 설정값 | 설명 |
| :--- | :--- | :--- |
| **Cascade Threshold** | `0.1` | 오염 전파 시뮬레이션 시 시맨틱 필터링 기준 |
| **Staining Threshold** | `0.2` | 실제 `STALE` 상태 전이 판정 기준 |
| **Max LLM Iterations** | `5` | 의도 분석 시 에이전트의 최대 사고/도구 호출 횟수 |
| **Validation Pass Score** | `80` | 패치 생성 후 자동 검증 통과 기준 점수 |
| **RAG Top-K** | `5` | 아티팩트 검색 시 참조할 최상위 문서 조규 수 |
